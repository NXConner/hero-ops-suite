...
export class AnalyticsService {
  ...
  async getAnalytics(): Promise<AnalyticsData> {
    ...
    try {
      ...
      const [gpsData, sensorData, weatherData] = await Promise.all([
        ...
        weatherService.getCurrentWeather(40.7128, -74.006), // Default location
      ]);
  ...
  private calculateFuelEfficiency(gpsData: GPSTrackingResponse[]): number {
    ...
    const speedEfficiency =
      avgSpeed >= 25 && avgSpeed <= 35 ? 100 : Math.max(60, 100 - Math.abs(avgSpeed - 30) * 2);
  ...
  private analyzeDefectIndicators(sensorData: SensorDataResponse[]): {
    ...
    const detectionAccuracy =
      sensorData.length > 0
        ? sensorData.filter((s) => s.quality === "good").length / sensorData.length
        : 0.95;
  ...
  private generateRecommendations(
    ...
    if (operationalEfficiency < 80) {
      recommendations.push(
        "🔧 Optimize fleet routing and scheduling to improve operational efficiency",
      );
    }
    ...
    if (defectData.criticalDefects > 0) {
      recommendations.push(
        "🚨 Prioritize immediate repairs for critical defects to prevent larger issues",
      );
    }
  ...
  private async getDailyOperationsTrend(gpsData: GPSTrackingResponse[]): Promise<number[]> {
    ...
    return Array.from({ length: 7 }, (_, i) => baseActivity + Math.floor(Math.random() * 5) - 2);
  }
  ...
  private async getMonthlyEfficiencyTrend(): Promise<number[]> {
    ...
    return Array.from({ length: 12 }, (_, i) => 85 + Math.floor(Math.random() * 15));
  }
