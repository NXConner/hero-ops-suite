...
export class WeatherService {
  ...
  async getUVIndex(lat: number, lon: number): Promise<number> {
    try {
      ...
      const response = await axios.get(`${API_CONFIG.WEATHER_BASE_URL}/uvi`, {
        params: {
          lat,
          lon,
          appid: API_CONFIG.WEATHER_API_KEY,
  ...
  async getAirQuality(lat: number, lon: number): Promise<any> {
    try {
      ...
      const response = await axios.get(`${API_CONFIG.WEATHER_BASE_URL}/air_pollution`, {
        params: {
          lat,
          lon,
          appid: API_CONFIG.WEATHER_API_KEY,
...
export class SensorDataService {
    ...
    sensorTypes.forEach((type, index) => {
      sensors.push({
        ...
        location: {
          ...
          longitude: -74.006 + (Math.random() - 0.5) * 0.005,
        },
  ...
  private getMockSensorHistory(
    ...
    const type = sensorId.includes("temperature")
      ? "temperature"
      : sensorId.includes("pressure")
        ? "pressure"
        : sensorId.includes("vibration")
          ? "vibration"
          : sensorId.includes("thickness")
            ? "thickness"
            : "compaction";
  ...
  private getMockValueForType(type: SensorDataResponse["type"]): number {
    switch (type) {
      case "temperature":
        return 150 + Math.random() * 100; // °F
      case "pressure":
        return 2000 + Math.random() * 1000; // PSI
      case "vibration":
        return Math.random() * 10; // Hz
      case "thickness":
        return 2 + Math.random() * 4; // inches
      case "compaction":
