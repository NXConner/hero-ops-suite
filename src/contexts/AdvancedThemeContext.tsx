...
export function AdvancedThemeProvider({
  ...
  const [choicePath, setChoicePathState] = useState<string | null>(() => {
    try {
      return localStorage.getItem("theme-choice-path") || null;
    } catch {
      return null;
    }
  });
  ...
  const [globalWallpaperOverride, setGlobalWallpaperOverrideState] =
    useState<ThemeWallpaper | null>(null);
  const [isGlobalWallpaperEnabled, setIsGlobalWallpaperEnabledState] = useState<boolean>(false);
  const [wallpaperProfiles, setWallpaperProfiles] = useState<
    { name: string; wallpaper: ThemeWallpaper }[]
  >([]);

  // Veteran module state
  const [isVeteran, setIsVeteranState] = useState<boolean>(() => {
    try {
      return localStorage.getItem("is-veteran") === "1";
    } catch {
      return false;
    }
  });
  const [veteranBranch, setVeteranBranchState] = useState<string>(() => {
    try {
      return localStorage.getItem("veteran-branch") || "";
    } catch {
      return "";
    }
  });
  const [isBranchWallpaperPersistent, setIsBranchWallpaperPersistentState] = useState<boolean>(
    () => {
      try {
        return localStorage.getItem("branch-wallpaper-persistent") === "1";
      } catch {
        return false;
      }
    },
  ...
  const [lowPowerMode, setLowPowerMode] = useState<boolean>(() => {
    try {
      return localStorage.getItem("low-power-mode") === "1";
    } catch {
      return false;
    }
  });
  ...
  const persistProfiles = useCallback((profiles: { name: string; wallpaper: ThemeWallpaper }[]) => {
    try {
      localStorage.setItem("global-wallpaper-profiles", JSON.stringify(profiles));
    } catch {
      /* ignore */
    }
  }, []);
  ...
  const saveToStorage = useCallback(
    (themes: Theme[], presetsData: ThemePreset[]) => {
      try {
        const config = {
          customThemes: themes,
          presets: presetsData,
    ...
    async (theme: Theme) => {
      ...
      `;
        ...
        document.documentElement.setAttribute(
          "data-performance",
          effectiveTheme.performance.quality,
        );
  ...
  const setChoicePath = useCallback(
    (path: string | null) => {
  ...
  const setGlobalWallpaperOverride = useCallback(
  ...
  const setIsGlobalWallpaperEnabled = useCallback(
    (enabled: boolean) => {
  ...
  const saveWallpaperProfile = useCallback(
    (name: string, wallpaper?: ThemeWallpaper | null) => {
      const wp = wallpaper ?? globalWallpaperOverride;
      if (!wp) return;
      const updated = [
        ...wallpaperProfiles.filter((p) => p.name !== name),
        { name, wallpaper: wp },
      ];
  ...
  const applyWallpaperProfile = useCallback(
    (name: string) => {
  ...
  const deleteWallpaperProfile = useCallback(
    (name: string) => {
    ...
    const map: Record<string, ThemeWallpaper> = {
      army: {
        ...
        gradient: {
          ...
          angle: 135,
          stops: [
            { color: { h: 90, s: 60, l: 28 }, position: 0 },
            { color: { h: 45, s: 80, l: 32 }, position: 100 },
          ],
        },
        overlay: { color: { h: 0, s: 0, l: 0 }, opacity: 0.35, blendMode: "multiply" },
      },
      navy: {
        type: "gradient",
        gradient: {
          ...
          angle: 135,
          stops: [
            { color: { h: 220, s: 70, l: 30 }, position: 0 },
            { color: { h: 50, s: 90, l: 40 }, position: 100 },
          ],
        },
        overlay: { color: { h: 0, s: 0, l: 0 }, opacity: 0.35, blendMode: "multiply" },
      },
      airforce: {
        ...
        gradient: {
          ...
          angle: 135,
          stops: [
            { color: { h: 210, s: 70, l: 40 }, position: 0 },
            { color: { h: 0, s: 0, l: 75 }, position: 100 },
          ],
        },
        overlay: { color: { h: 0, s: 0, l: 0 }, opacity: 0.3, blendMode: "multiply" },
      },
      marines: {
        type: "gradient",
        gradient: {
          ...
          angle: 135,
          stops: [
            { color: { h: 350, s: 80, l: 40 }, position: 0 },
            { color: { h: 45, s: 85, l: 45 }, position: 100 },
          ],
        },
        overlay: { color: { h: 0, s: 0, l: 0 }, opacity: 0.35, blendMode: "multiply" },
      },
      coastguard: {
        ...
        gradient: {
          ...
          angle: 135,
          stops: [
            { color: { h: 200, s: 75, l: 40 }, position: 0 },
            { color: { h: 10, s: 85, l: 48 }, position: 100 },
          ],
        },
        overlay: { color: { h: 0, s: 0, l: 0 }, opacity: 0.3, blendMode: "multiply" },
      },
      spaceforce: {
        ...
        gradient: {
          ...
          angle: 135,
          stops: [
            { color: { h: 250, s: 60, l: 35 }, position: 0 },
            { color: { h: 210, s: 60, l: 30 }, position: 100 },
          ],
        },
        overlay: { color: { h: 0, s: 0, l: 0 }, opacity: 0.4, blendMode: "multiply" },
      },
    };
    return map[branch] || null;
  }, []);

  const applyBranchWallpaperIfNeeded = useCallback(
    (branch: string, enabled: boolean) => {
      if (enabled && branch) {
  ...
  const setIsVeteran = useCallback((value: boolean) => {
    ...
    try {
      localStorage.setItem("is-veteran", value ? "1" : "0");
    } catch (_e) {
      /* ignore */
    }
  }, []);
  ...
  const setVeteranBranch = useCallback((branch: string) => {
    ...
    try {
      localStorage.setItem("veteran-branch", branch);
    } catch (_e) {
      /* ignore */
    }
  }, []);
  ...
  const setIsBranchWallpaperPersistent = useCallback((enabled: boolean) => {
    ...
    try {
      localStorage.setItem("branch-wallpaper-persistent", enabled ? "1" : "0");
    } catch (_e) {
      /* ignore */
    }
  }, []);
  ...
  const setLowPower = useCallback(
    (enabled: boolean) => {
      setLowPowerMode(enabled);
      try {
        localStorage.setItem("low-power-mode", enabled ? "1" : "0");
      } catch {
        /* ignore */
      }
      if (currentTheme) applyTheme(currentTheme);
  ...
  return (
    <AdvancedThemeContext.Provider value={contextValue}>{children}</AdvancedThemeContext.Provider>
  );
}
