...
export default function ContractorPortal() {
  ...
  const update = async (job_id: string, patch: any) => {
    await fetch(`${API}/jobs`, {
      ...
      body: JSON.stringify({ job_id, ...patch }),
    });
    await load();
  };
        ...
        {jobs.map((j) => (
          <div key={j.job_id} className="border rounded p-4 flex items-center justify-between">
            <div>
              ...
              <div>
                Scan: {j.scan_id || "—"} | Status: {j.status}
              </div>
            </div>
            <div className="flex gap-2">
              <button
                className="bg-green-600 text-white px-3 py-1 rounded"
                onClick={() => update(j.job_id, { status: "accepted" })}
              >
                Accept
              </button>
              <button
                className="bg-yellow-600 text-white px-3 py-1 rounded"
                onClick={() => update(j.job_id, { status: "in_progress" })}
              >
                Start
              </button>
              <button
                className="bg-blue-600 text-white px-3 py-1 rounded"
                onClick={() => update(j.job_id, { status: "completed" })}
              >
                Complete
              </button>
            </div>
