...
const VehicleChecklists: React.FC<Props> = ({ vehicles }) => {
  ...
  const [maintenanceItems, setMaintenanceItems] =
    useState<ChecklistItem[]>(DEFAULT_MAINTENANCE_ITEMS);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (inspections && inspections.length > 0) {
        const latest = inspections.sort((a: VehicleChecklistRecord, b: VehicleChecklistRecord) =>
      ...
      if (maints && maints.length > 0) {
        const latest = maints.sort((a: VehicleChecklistRecord, b: VehicleChecklistRecord) =>
          b.createdAt.localeCompare(a.createdAt),
        )[0];
              ...
              {inspectionItems.map((item) => (
                <label key={item.id} className="flex items-center gap-2 text-xs text-slate-200">
                  <Checkbox
                    ...
                    onCheckedChange={(v) => toggleItem("inspection", item.id, Boolean(v))}
                  />
            ...
            <div className="grid grid-cols-1 gap-2">
              {maintenanceItems.map((item) => (
                <label key={item.id} className="flex items-center gap-2 text-xs text-slate-200">
                  <Checkbox
                    checked={item.checked}
                    onCheckedChange={(v) => toggleItem("maintenance", item.id, Boolean(v))}
                  />
                  <span>{item.label}</span>
                </label>
              ))}
