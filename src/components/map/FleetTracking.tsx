...
const FleetTracking: React.FC<FleetTrackingProps> = ({ terminologyMode, isVisible }) => {
  ...
  const convertGPSToVehicle = (gpsData: GPSTrackingResponse): Vehicle => {
    return {
      ...
      status:
        gpsData.status === "active" ? "active" : gpsData.status === "idle" ? "idle" : "offline",
  ...
  const convertGPSToEmployee = (gpsData: GPSTrackingResponse): Employee => {
    return {
      ...
      status: gpsData.isMoving ? "clocked-in" : gpsData.status === "idle" ? "break" : "clocked-out",
      phoneUsage: {
  ...
  const generateEnhancedMockVehicles = (): Vehicle[] => [
    {
      ...
      type: "truck",
      location: { lat: 40.7128, lng: -74.006, heading: 45, speed: 25 },
      status: "active",
      driver: {
        id: "e1",
        name: "John Smith",
    ...
    },
    {
      ...
      type: "paver",
      location: { lat: 40.72, lng: -74.01, heading: 90, speed: 15 },
      status: "active",
      lastUpdate: new Date(),
      fuel: 45,
      maintenance: {
    ...
    },
    {
      ...
      type: "roller",
      location: { lat: 40.715, lng: -74.002, heading: 180, speed: 0 },
      status: "idle",
      lastUpdate: new Date(),
      fuel: 60,
    },
  ...
  const generateEnhancedMockEmployees = (): Employee[] => [
    {
      ...
      location: { lat: 40.714, lng: -74.005, accuracy: 3 },
      status: "clocked-in",
    ...
    },
    {
      ...
      location: { lat: 40.716, lng: -74.008, accuracy: 4 },
      status: "clocked-in",
  ...
  useEffect(() => {
    ...
    vehicles.forEach((vehicle) => {
      const marker = mapMethods.addMarker(vehicle.location.lng, vehicle.location.lat, {
        color:
    ...
    employees.forEach((employee) => {
      const marker = mapMethods.addMarker(employee.location.lng, employee.location.lat, {
        color:
                ...
                {alerts.map((alert) => (
                  <div key={alert.id} className="bg-slate-800 p-2 rounded text-xs">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-slate-400">{alert.timestamp.toLocaleTimeString()}</span>
                    </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  ...
                  <Button variant="ghost" size="sm" onClick={resetPlayback} className="p-1">
              ...
              {isConnectedToAPI ? (
                <div className="text-xs text-slate-300">Real-time GPS data available</div>
              ) : (
                <div className="text-xs text-yellow-400">Using simulated data</div>
              )}

              <div className="text-xs text-slate-400">Data points: {realTimeData.length}</div>
            </div>
          </TabsContent>
