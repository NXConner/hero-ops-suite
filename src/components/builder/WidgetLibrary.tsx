...
export default function WidgetLibrary({ widgets, onAdd }: WidgetLibraryProps) {
  const iconFor = (id: string) => {
    switch (id) {
      case 'fleet': return <Users className="h-4 w-4 text-cyan-400" />;
      case 'weather': return <Cloud className="h-4 w-4 text-blue-400" />;
      case 'pavement': return <Activity className="h-4 w-4 text-orange-400" />;
      case 'mission': return <Target className="h-4 w-4 text-green-400" />;
      default: return null;
    }
  };
