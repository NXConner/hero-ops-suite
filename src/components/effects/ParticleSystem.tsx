...
function updateParticle(
  ...
  particle.opacity = Math.max(0, lifeRatio * (Math.random() * (effect.opacity.max - effect.opacity.min) + effect.opacity.min));
  ...
  switch (effect.type) {
    ...
    case 'geometric':
      ...
      const radius = Math.sqrt(Math.pow(particle.x - centerX, 2) + Math.pow(particle.y - centerY, 2));
