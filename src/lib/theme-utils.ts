...
export function generateColorScheme(baseColor: ThemeColor, type: 'monochromatic' | 'complementary' | 'triadic' | 'analogous'): ThemeColor[] {
  switch (type) {
    case 'monochromatic':
      return [
...
export function animationToCSS(animation: Animation): string {
  const iterations = animation.iterations === 'infinite' ? 'infinite' : animation.iterations.toString();
...
export function shouldEnableEffect(effect: string, performance: 'low' | 'medium' | 'high' | 'ultra'): boolean {
  const effectLevels = {
...
export function createThemeVariant(baseTheme: Theme, colorAdjustments: Partial<ThemeColor>): Theme {
  const adjustedColors = Object.entries(baseTheme.colors).reduce((acc, [key, color]) => {
    acc[key as keyof typeof baseTheme.colors] = {
      h: colorAdjustments.h !== undefined ? (color.h + colorAdjustments.h) % 360 : color.h,
      s: colorAdjustments.s !== undefined ? Math.max(0, Math.min(100, color.s + colorAdjustments.s)) : color.s,
      l: colorAdjustments.l !== undefined ? Math.max(0, Math.min(100, color.l + colorAdjustments.l)) : color.l,
